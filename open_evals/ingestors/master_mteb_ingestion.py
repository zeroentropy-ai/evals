...
def extract_document_content(doc_data: str | dict) -> str:
...
def extract_query_content(query_data: str | dict) -> str:
...
def extract_score_value(score_data: int | float | dict | str) -> float:
...
def find_best_split(available_splits: list[str], requested_split: str, language: str | None) -> str:
...
class MasterMtebIngestor(BaseIngestor):
    ...
    def ingest(self) -> tuple[list[Query], list[Document], list[QRel]]:
        ...
        if self.language == "multilingual":
            available_splits = list(task.queries.keys()) if hasattr(task, 'queries') and task.queries else []
            print(f"Language is multilingual, combining all {len(available_splits)} splits")
        ...
        try:
            ...
            if available_splits:
                actual_split = find_best_split(available_splits, self.split, self.language)
                ...
                raise ValueError(f"No data splits available for {self.task_name}")
        ...
        if corpus_dict and len(corpus_dict) > 0:
            ...
            if isinstance(first_value, dict) and len(first_value) > 0:
                ...
                inner_sample = list(first_value.values())[0]
                if isinstance(inner_sample, dict) and not ('text' in first_value or 'title' in first_value):
        ...
        if relevant_docs_dict and len(relevant_docs_dict) > 0:
            ...
            if isinstance(first_value, dict) and len(first_value) > 0:
                ...
                if isinstance(inner_sample, dict):  # Should be {doc_id: score} mapping
                    ...
                    for inner_split, inner_qrels in relevant_docs_dict.items():
    ...
    def _combine_all_splits(self, task, available_splits: list[str]) -> tuple[list[Query], list[Document], list[QRel]]:
                        ...
                        for doc_id, doc_data in inner_corpus.items():
                            if doc_id not in seen_doc_ids:
                                ...
                                all_documents.append(Document(
                                    id=doc_id,
                                    content=extract_document_content(doc_data),
                                    metadata={"dataset": self.dataset_name, "language": "multilingual", "split": f"{split}_{inner_split}"}
                                ))
                else:
                    # Direct structure
                    for doc_id, doc_data in corpus_dict.items():
                        if doc_id not in seen_doc_ids:
                            ...
                            all_documents.append(Document(
                                id=doc_id,
                                content=extract_document_content(doc_data),
                                metadata={"dataset": self.dataset_name, "language": "multilingual", "split": split}
                            ))
        ...
        for split in available_splits:
            try:
                ...
                if relevant_docs_dict and isinstance(list(relevant_docs_dict.values())[0], dict):
                    ...
                    if isinstance(first_value, dict) and not any(isinstance(v, (int, float, str)) for v in first_value.values()):
                        # Nested structure: process all inner qrels
                        for inner_split, inner_qrels in relevant_docs_dict.items():
                            for query_id, doc_scores in inner_qrels.items():
                                if query_id in collected_query_ids:  # Only if we have this query
                                    for doc_id, score in doc_scores.items():
                                        if doc_id in collected_doc_ids:  # Only if we have this document
        ...
        print(f"Combined: {len(all_queries)} queries, {len(all_documents)} documents, {len(all_qrels)} qrels")
        ...
        valid_qrels = []
        seen_qrel_pairs = set()  # Track (query_id, document_id) pairs to avoid duplicates
        
        for qrel in all_qrels:
            if qrel.query_id in collected_query_ids and qrel.document_id in collected_doc_ids:
        ...
        if len(valid_qrels) != len(all_qrels):
            print(f"Filtered qrels: {len(all_qrels)} -> {len(valid_qrels)} (removed {len(all_qrels) - len(valid_qrels)} duplicates/invalid references)")
    ...
    async def load_queries(self) -> list[Query]:
        ...
        try:
            ...
            if available_splits:
                ...
                raise ValueError(f"No query splits available for {self.task_name}")
    ...
    async def load_corpus(self) -> list[Document]:
        ...
        try:
            ...
            available_splits = list(task.corpus.keys()) if hasattr(task, 'corpus') and task.corpus else []
            print(f"WARNING: Split '{self.split}' not found for {self.task_name}")
            print(f"Available splits: {available_splits}")
            if available_splits:
                actual_split = find_best_split(available_splits, self.split, self.language)
                ...
                raise ValueError(f"No corpus splits available for {self.task_name}")
    ...
    async def load_qrels(self) -> list[QRel]:
        ...
        try:
            ...
            available_splits = list(task.relevant_docs.keys()) if hasattr(task, 'relevant_docs') and task.relevant_docs else []
            print(f"WARNING: Split '{self.split}' not found for {self.task_name}")
            print(f"Available splits: {available_splits}")
            if available_splits:
                actual_split = find_best_split(available_splits, self.split, self.language)
                ...
                raise ValueError(f"No qrel splits available for {self.task_name}")
