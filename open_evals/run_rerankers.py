...
from open_evals.run_embeddings import RETRIEVAL_METHOD, MERGE_STATUS

from utils import read_num_lines_pbar, wrap_sem
...
async def rerank_ze_results(
    ...
    all_results = [
        ...
        for reranker in rerankers.values() if reranker != VOYAGE_RERANKER or voyage_rank
    ]
...
async def rerank_dataset(
    ...
    with open(input_file_path) as f, open(output_file_path, "a") as f_write:
        ...
        async def wrapped_process_line(line: str, voyage_rank: bool = False) -> None:
            ze_results = ZEResults.model_validate_json(line)
            if ze_results.query_id not in processed_query_ids:
                scored_ze_results = await rerank_ze_results(
                    ze_results,
                    rerankers,
                    voyage_rank=voyage_rank,
                )
                f_write.write(scored_ze_results.model_dump_json() + "\n")
            pbar.update(1)

        for i, line in enumerate(f):
            task = asyncio.create_task(wrapped_process_line(line, i % 4 == 0))
