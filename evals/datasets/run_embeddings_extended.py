...
from evals.datasets.common import Document, QRel, Query, ZEDataset, ZEResults
from evals.datasets.run_ingestors import EVAL_DATASETS
...
async def get_openai_small_embeddings(
    ...
    for j in range(0, num_docs, DOT_PRODUCT_BATCH_SIZE):
        ...
        dot_products[:, j:end] = query_embeddings @ document_embeddings[j:end].T
        pbar.update(end - j)
    pbar.close()
...
def get_bm25_embeddings(
    ...
    tokenized_docs = [doc.lower().split() for doc in tqdm(document_texts, desc="Tokenizing")]
    ...
    # Get top k documents for each query
    print(f"Finding top {k} documents...")
    top_indices = np.argpartition(similarity_scores, -k, axis=1)[:, -k:]  # (num_queries, k)
    sorted_top_indices = np.argsort(
...
async def get_hybrid_embeddings(
    ...
    for query_idx in range(num_queries):
        ...
        sorted_embedding_tuples = sorted(embedding_doc_scores.items(), key=lambda x: x[1], reverse=True)
        sorted_bm25_tuples = sorted(bm25_doc_scores.items(), key=lambda x: x[1], reverse=True)
        ...
        for i in range(len(sorted_bm25_tuples)):
            doc_scores[sorted_bm25_tuples[i][0]]  = doc_scores.get(sorted_bm25_tuples[i][0], 0) + (1/(i+1))
...
async def generate_embeddings(
    ...
    *,
    ...
    embeddings_cache = (
        dc.Cache(
            dataset.file_path("embeddings_cache.db"),
    ...
    with open(dataset.file_path("ze_results.jsonl"), "w") as f:
        queries_processed = 0
        queries_skipped = 0
        ...
        for query_index, query in enumerate(queries):
            ...
            if INCLUDE_RELEVANT_DOCS:
                ...
                if len(qrel_indices) > k:
                    ...
                    final_scores = [0.0] * k  # Will be set below based on method results
                else:
                    # Get natural results, remove duplicates with relevant docs, add relevant docs
                    natural_indices = [i for i in query_top_sorted_indices if i not in qrel_indices]
                ...
                score_lookup = {}
                for i, (idx, score) in enumerate(zip(query_top_sorted_indices, query_similarity_scores)):
                ...
                sorted_pairs = sorted(zip(final_indices, final_scores), key=lambda x: x[1], reverse=True)
                ...
                relevant_in_top_k = [i for i in query_top_sorted_indices if i in qrel_indices]
    ...
    if queries_skipped > 0:
        print(f"Skipped {queries_skipped} queries with no relevant documents in top {k}")
