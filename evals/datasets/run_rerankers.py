...
from evals.datasets.common import ZEDataset, ZEResults
from evals.datasets.run_ingestors import EVAL_DATASETS
from evals.ai import modal_pointwise_score
from evals.utils import read_num_lines_pbar, wrap_sem
...
RERANKERS: dict[str, AIRerankerType] = {
    ...
    "zeroentropy-small": "https://npip99--ze-rerank-small-v0-3-0-model-endpoint.modal.run/",
    "zeroentropy-large": "https://npip99--ze-rerank-v0-3-0-model-endpoint.modal.run/",
}
RERANK_MAX_TOKENS = 4096
RERANK_MAX_BATCH_CHARACTERS = 64_000
SAVE_NAME = "latest_ze_results.jsonl"
...
async def process_query(
    ...
    for batch in batches:
        if isinstance(reranker, str):
            reranked_scores = await wrap_sem(
                modal_pointwise_score(
                    [(query, corpus_text) for corpus_text in batch],
                    url=reranker,
                ),
                RERANK_SEMAPHORE,
            )
...
async def rerank_ze_results(
    ...
    for reranker_name, results in zip(rerankers.keys(), all_results, strict=True):
        for document, result in zip(ze_results.documents, results, strict=True):
...
async def rerank_dataset(
    ...
    num_lines = read_num_lines_pbar(dataset.ze_results_path, display_name=dataset.id)
    ...
    with open(dataset.ze_results_path) as f, open(output_file_path, "a") as f_write:
