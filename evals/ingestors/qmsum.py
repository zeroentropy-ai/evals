...
class QMSumIngestor(BaseIngestor):
    ...
    def ingest(self) -> tuple[list[Query], list[Document], list[QRel]]:
        ...
        all_queries = test_data[
            "output"
        ]  # surprisingly. The output is much shorter than the input "document"
        all_query_ids = test_data["id"]
        all_doc_info = test_data["input"]
        for document_id, (query, query_id, doc) in tqdm(
            enumerate(zip(all_queries, all_query_ids, all_doc_info, strict=True)),
            desc="Datapoints",
        ):
