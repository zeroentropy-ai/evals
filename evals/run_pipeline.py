...
async def main(start_action: ACTIONS = "ingestors", end_action: ACTIONS = "ndcg") -> None:
    if ORDER[start_action] <= ORDER["ingestors"] and ORDER[end_action] >= ORDER["ingestors"]:
        run_ingestors(ingestors=INGESTORS, max_queries=MAX_QUERIES)
    if ORDER[start_action] <= ORDER["embeddings"] and ORDER[end_action] >= ORDER["embeddings"]:
        await run_embeddings(
            ingestors=INGESTORS,
            retrieval_method=RETRIEVAL_METHOD,
            include_relevant_docs=INCLUDE_RELEVANT_DOCS,
        )
    if ORDER[start_action] <= ORDER["rerankers"] and ORDER[end_action] >= ORDER["rerankers"]:
        await run_rerankers(
            ingestors=INGESTORS,
            rerankers=RERANKERS,
            retrieval_method=RETRIEVAL_METHOD,
